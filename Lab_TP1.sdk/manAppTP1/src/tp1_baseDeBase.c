...
int configGPIOs () {
  ...
  XGpio_SetDataDirection (&GpioAXI,1,0);
  XGpio_SetDataDirection (&GpioAXI,2, 1);
...
void setTimerAndIntr() {
  ...
  XScuGic_Connect(&InterruptController,XPAR_AXI_GPIO_0_INTERRUPT_PRESENT,(Xil_InterruptHandler)TimerIntrHandler,&Timer);

  /* enable the interrupt for the Timer at GIC */
  XScuGic_Enable(&InterruptController,XPAR_AXI_GPIO_0_INTERRUPT_PRESENT);
...
int main()
{
    ...
    while (1) {
       ...
       if (value == 1)
       	   XGpioPs_WritePin(&GpioMIO,LD9,0xFFFFFFFF);
       else
       	   XGpioPs_WritePin(&GpioMIO,LD9,0x00000000);
       ...
       if (value == 1) {
  	   ...
  	   value = XGpio_DiscreteRead (&GpioAXI,1 );
